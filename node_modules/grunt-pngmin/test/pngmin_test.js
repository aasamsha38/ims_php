...
exports.pngmin = {
    ...
    error_test: function (test) {
        test.expect(1);

        // Prep to run grunt directly...
        ...
        // Call the grunt test directly and ensure we're getting a non-zero exit status.
        exec('grunt pngmin:error_test', execOptions, function(error, stdout) {
            test.ok(error && error.code !== 0, 'Ensure pngmin:error_test results in non-zero exit status. Exec result was: ' + JSON.stringify({error: error, stdout: stdout}));
            test.done();
        });
    },
};
